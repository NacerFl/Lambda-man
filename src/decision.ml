...
*)
(*prend une position et une liste et retourne la liste des segments formés par cette position et les elts de la liste*)

  
(*verifie si un segment est en intersection avec un segments d'une liste de segment *)
let est_inter (x,y) l =
  let rec aux l =
  match l with
  |[] -> false
  |(a,b)::q -> if (Space.segment_intersects (x,y) (a,b)) then true
  else aux q
in aux l

(*prend une position, une liste de position et une liste de segments l2, et 
  retourne la liste des edges formés par la position et la liste des positions
   qui ne sont pas en intersection avec un segment de l2
*)


let pos_segment (x,y) l =
  let rec aux (x,y) l res =
  match l with
  |[] -> res
  |(x',y')::q -> aux (x,y) q (res@[((x,y),(x',y'))])
in aux (x,y) l []

(*retourne la liste de tout les edges qui ne sont pas en intersection avec les segments de l2*)
let list_seg list_pos = 
  let rec aux list_pos res =
    match list_pos with 
    |[]->res
    |t::q -> aux q (res@(pos_segment t q))
  in aux list_pos []
...
let segment_du_graph list_seg list_seg_enfer=
  let rec aux list_seg res =
  match list_seg with
  |[] -> res
  |(x,y)::q -> if est_inter (x,y) list_seg_enfer then aux q res
  else aux q ((x,y)::res)
in aux list_seg []

(* pour convertir une distance en float *)
let float_distance d = match d with
  |Distance a -> a
;;

  let list_edge list_seg = 
  let rec aux list_seg res =
   match list_seg with
    |[] -> res
    |(t,t')::q -> let dist =  float_distance (Space.dist2 t t') in
      aux q ((t, t', dist)::res)
  in aux list_seg []



(*prend la liste des segments des polygones et retourne la liste des sommets des polygones*)
let list_pos_bouche_enfert l =
  let rec aux l res=
  match l with 
  |[] -> res
  |(p1,p2)::q ->aux q (p1::p2::res)   
in List.rev (aux l [])
...
let visibility_graph observation memory =
  let list_seg_poly = hell_segments (World.world_of_observation observation) in
  let node_list = (World.tree_positions (observation.trees))@[observation.spaceship; observation.position]@(list_pos_bouche_enfert list_seg_poly) in
  
  let list_seg = set_segments node_list in
  let seg_du_graph = segment_du_graph list_seg list_seg_poly in
  let list_edge = list_edge seg_du_graph in
  Graph.make node_list list_edge



(*les modules pour la realisation de la file de priorité*)

(*Module representant les positions*)
module Cle =
  struct
    type t = Graph.node
    let compare = compare
...
(*Module representant la priorité sur les positions*)
module Priorite =
...
module Q= PriorityQueue.Make(Cle) (Priorite)
...
let init_tab node node_list =
  try
  let rec aux l res_d res_p = 
    match l with
      |[] -> (res_d, res_p)
      |t::q -> if node = t then aux q ((t,0.)::res_d) ((t,(infinity,infinity))::res_p)
      else aux q ((t,Priorite.infinity)::res_d) ((t,(infinity,infinity))::res_p)
  in aux node_list [] []
  with _ -> failwith "erreur d'initialisation"

(*fonctions de mise à jour des distances et des predecesseurs *)

let mise_a_jour l node =
  let (a,b) = node in
  List.filter (fun x -> a!=(fst x)) l

let pred_mise_a_jour l node =
  let (a,b) = node in
  List.filter (fun x -> a!=(fst x)) l

(*fonction qui met à jour les distances, predecesseurs et la file des aretes sortants du minimum extrait*)

let mise_a_jour_dist list_d list_p graph q min =
  try
  let rec aux a res_d res_p res_q = match a with
    |[] -> (res_d, res_p, res_q)
    |(a,b,c)::q ->
    let y = List.find (fun x -> b=(fst x)) res_d in
    if snd y > (snd min) +. c then aux q ((fst y,snd min +. c)::(mise_a_jour res_d y)) ((fst y,fst min)::(pred_mise_a_jour res_p y)) (Q.decrease res_q (fst y) (snd min +. c))
    else aux q res_d res_p res_q
  in aux (Graph.out graph (fst min)) list_d list_p q
  with _ -> failwith "mise_a_jour"

(* mise a jour de tout les noeuds minimum extraits successivement *)

let mise_a_jour_djisktra q list_d list_p graph  =
  try
    let rec aux taille res_d res_p res_q = 
      match taille with
        |0 -> (res_d, res_p)
        |_ -> let (p,n) = Option.get (Q.get_min res_q) in
        let min = (n,p) in
        let queue = Q.remove_min res_q in
        let (dist,pred,queue) = mise_a_jour_dist res_d res_p graph queue min in
          aux (Q.length queue) dist pred queue
      in aux (Q.length q) list_d list_p q
  with _ -> failwith "erreur"



(*insertion des noeuds dans la file *)

let insertion queue list =
  try
    let rec aux l res_q = match l with
      |[] -> res_q
      |(n,p)::q -> aux q (Q.insert res_q n p)
    in aux list queue
  with _ -> failwith "erreur d'insertion"
 

(*la fonction djikstra retourne les distances et les predecesseurs *)

let djikstra graph source =

  try
    let nd_list = Graph.nodes graph in
    let (dist,pred) = init_tab source nd_list in
    let queue = Q.empty in
    let q = insertion queue dist in              
    mise_a_jour_djisktra q dist pred graph
...
let shortest_path graph source target : path =
  ...
  try
    let rec aux cible res_path =
      let (node,succ) = 
        try 
          List.find (fun x -> cible=(fst x)) pred 
        with Not_found -> failwith "pas de cible" in
      if succ = (infinity,infinity) then res_path
      else aux succ (node::res_path)
    in aux target []
  with _ -> failwith "erreur shortest_path"
...
*)
(*verifie si un chemin est correct*)
let chemin_correct path l=
let rec aux path =
  match path with
  | [] -> true 
  | [x]-> true
  | [x ;y] -> not (est_inter (x,y) l)
  | x :: y :: q -> 
    if est_inter (x,y) l then false
    else aux (y::q)
in aux path

let plan visualize observation memory =
  if visualize then Visualizer.show_graph (visibility_graph observation memory) ;
  match memory.objective with
    Initializing ->
      let grf = visibility_graph observation memory in 
      let targets = (World.tree_positions observation.trees) @ [observation.spaceship] in  
      let path =  (shortest_path grf observation.position (List.hd targets)) in
     {known_world = memory.known_world; graph = grf;  objective = GoingTo(path,[]); targets = targets; }
   |Chopping ->  memory
   |GoingTo(c1, c2) -> 
      let grf = visibility_graph observation memory in
      let seg = hell_segments (World.world_of_observation observation) in
      if chemin_correct (observation.position :: c1) seg then {memory with graph = grf} (*Printf.eprintf "correctes\n%!";memory*)
      else
        let path =  (shortest_path grf observation.position (List.hd memory.targets)) in
        {memory with graph = grf;  objective = GoingTo(path,[]);}        
...
(**
   ...
   (Space.angle_of_float ang, Space.speed_of_float vitesse)

*)

(*calcul de l'angle*)
let angle p1 p2 = Space.angle_of_float (atan2 (y_ p2 -. y_ p1) (x_ p2 -. x_ p1))
  
let next_action visualize observation memory =
  match memory.objective with
  |Initializing -> failwith "la planification a echouée" (*plan visualize observation memory*)
  |Chopping ->
    begin
      let tree = World.tree_at observation.trees observation.position in
      match tree with
      |None ->
        let grf = visibility_graph observation memory in
        Move (observation.angle, observation.max_speed),memory
      |Some t ->
        if t.branches > 0 then
          ChopTree,memory
        else
          let tar = List.tl memory.targets in
          let ang = angle observation.position (List.hd tar)  in
          let vitesse = observation.max_speed in
          let grf = visibility_graph observation memory in
          Move (ang,vitesse), {memory with graph=grf; objective = GoingTo(tar,[]);targets = tar;}
    end 
  |GoingTo(c1, c2) -> 
    let tree = World.tree_at observation.trees observation.position in
    match tree with
    |None -> 
       let ang = angle observation.position (List.hd c1) in
        Move(ang, observation.max_speed), memory
    |Some t ->
      let grf = visibility_graph observation memory in
      Move(observation.angle, Space.speed_of_float 0.),
      {memory with graph = grf; objective = Chopping}
